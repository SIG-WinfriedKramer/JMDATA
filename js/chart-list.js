...
$(document).ready(function () {
    ...
    for (var e = 0, l = EXAMPLES.length; l > e; ++e) {
        var r = EXAMPLES[e].title || "鏈懡鍚嶅浘琛�", i = (EXAMPLES[e].subtitle || "鐐瑰嚮鏌ョ湅璇︽儏", $('<div class="col-lg-3 col-md-4 col-sm-6"></div>')), c = $('<div class="chart"></div>');
        $("#chart-row-" + EXAMPLES[e].type).append(i.append(c)), $link = $('<a class="chart-link" href="demo.html#' + EXAMPLES[e].id + '"></a>'), c.append($link), $link.append('<h4 class="chart-title">' + r + "</h4>"), $chartArea = $('<img class="chart-area" data-original="' + GALLERY_THUMB_PATH + EXAMPLES[e].id + '.png" />'), $link.append($chartArea)
    }
